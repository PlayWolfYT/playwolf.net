...
export function createBlaze(
  ...
  function render() {
    ...
    gl!.uniform1f(firePass.uniforms.uSparkDensity, Math.max(config.sparkDensity, 0.05));
    gl!.uniform1f(firePass.uniforms.uSparkSize, Math.max(config.sparkSize, 0.05));
    gl!.uniform1i(
...
export function Blaze({ children, className, style, ...options }: BlazeProps) {
  ...
  useEffect(() => {
    ...
    instanceRef.current = createBlaze({ source, content, output }, initialOptions);
